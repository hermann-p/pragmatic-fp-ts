...
const record = <T extends Record<string, any>>(schema: {
  ...
  const recordType = Object.entries(schema)
    ...
    .join(", ");
  return (x: unknown): Either<T, Error> => {
...
const dictionary =
  <T>(decode: ValidatorFn<T>): Validator<Record<string, T>> =>
  (x: unknown): Either<Record<string, T>, Error> => {
    const decodeDict = (o: any) =>
      Object.fromEntries(Object.entries(o).map(([k, v]) => [k, decode(v)]));
    return isPlainObject(x)
      ? liftValue(decodeDict(x))
      : left(new Error(`expected Dictionary<${__getName(decode)}>, got ${typeof x} ${x}`));
  };
