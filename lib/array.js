...
function countBy(counter) {
    return (countable) => groupBy(counter)(countable).bind(object_1.mapValues((v) => v.length));
}
exports.countBy = countBy;
exports.flatten = (coll) => {
    const result = [];
    Maybe_1.maybe(coll).bind((c) => c.forEach((value) => {
    ...
    return Maybe_1.just(result);
};
exports.flatMap = (fn) => (coll) => Maybe_1.maybe(coll)
    .bind(exports.map(fn))
    .bind(exports.flatten);
