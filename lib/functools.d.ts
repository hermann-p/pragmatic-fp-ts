 ...
 */
export declare const when: <T>(pred: Predicate<T>) => (ifFn: Isomorphism<T>) => (value: MaybeType<T>) => Maybe<T>;
/**
 * applies elseFn only when pred is false
 */
export declare const unless: <T>(pred: Predicate<T>) => (elseFn: Isomorphism<T>) => (value: MaybeType<T>) => Maybe<T>;
